...
#define CircleCenter_1    (130)    // 圆弧内径
#define CircleCenter_2    (150)    // 圆弧外径
#define CircleCenter_3    (CircleCenter_2 + 10)    //  不大于 CircleSize / 2

#define CircleSize    320    // 圆形显示区域的大小
#define Circle_X      (427)    // 圆心X坐标
#define Circle_Y      (265)   // 圆心Y坐标
...
void Circle_Paint(HWND hwnd, HDC hdc)
{
  ...
  /* 计算左边小圆圈的坐标 */
  vertex_x =  Circle_X - ((CircleCenter_3 - CircleCenter_2) / 2 + CircleCenter_2) * sin(3.14/4); 
  vertex_y =  Circle_Y + ((CircleCenter_3 - CircleCenter_2) / 2 + CircleCenter_2) * cos(3.14/4); 
  FillCircle(hdc, vertex_x, vertex_y, (CircleCenter_3 - CircleCenter_2) / 2);     // 左边小圆圈
  
  vertex_x =  Circle_X + ((CircleCenter_3 - CircleCenter_2) / 2 + CircleCenter_2) * sin(3.14/4) ;
  FillCircle(hdc, vertex_x+2, vertex_y, (CircleCenter_3 - CircleCenter_2) / 2);     // 右边小圆圈
  ...
  rc.w = 24*4 - 10;
  rc.h = 30;rc.x +=15; 
  rc.y = CircleSize/2 - rc.h/2 + CircleCenter_3 + 64;
...
//        rc.h = CircleSize * 2;
    ...
    case WM_PAINT:
    {
      HDC hdc;
      PAINTSTRUCT ps;
      RECT rc = {Circle_X-CircleSize/2, Circle_Y-CircleSize/2, CircleSize, CircleSize};
      ...
      SetBrushColor(hdc, MapRGB(hdc, 1, 218, 254));
      FillCircle(hdc, Circle_X,Circle_Y, CircleSize/2);
