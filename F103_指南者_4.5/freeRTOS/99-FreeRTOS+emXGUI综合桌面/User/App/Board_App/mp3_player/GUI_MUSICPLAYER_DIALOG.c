...
static void button_owner_draw(DRAWITEM_HDR *ds)
{
  ...
  GetWindowText(ds->hwnd,wbuf,36); //获得按钮控件的文字  
...
static void _music_textbox_OwnerDraw(DRAWITEM_HDR *ds) //绘制一个按钮外观
{
  ...
  GetWindowText(hwnd, wbuf, 64); //获得按钮控件的文字
...
//	power_draw_scrollbar(hwnd, hdc, color_bg, RGB888( 250, 250, 250), RGB888( 255, 255, 255));
   ...
   SendMessage(hwnd, SBM_GETTRACKRECT, 0, (LPARAM)&rc);   
	...
	if (ds->State & SST_THUMBTRACK)//按下
	{
			power_draw_scrollbar(hwnd, hdc, color_bg, RGB888(	20, 155, 158), RGB888(12, 38, 41));
...
//                        0, 0, 80, 80, hwnd, ID_EXIT, NULL, NULL); 
         ...
         wnd_lrc4 = CreateWindow(TEXTBOX, L" ", WS_OWNERDRAW | WS_VISIBLE|WS_CLIPSIBLINGS, 
                                50, 304, 700, 30, hwnd, ID_TEXTBOX_LRC4, NULL, NULL);  
         SendMessage(wnd_lrc4,TBM_SET_TEXTFLAG,0,DT_VCENTER|DT_CENTER|DT_BKGND); 
         wnd_lrc5 = CreateWindow(TEXTBOX, L" ", WS_OWNERDRAW | WS_VISIBLE|WS_CLIPSIBLINGS, 
         ...
         CreateWindow(BUTTON,L"歌曲文件名",WS_OWNERDRAW|WS_TRANSPARENT|WS_VISIBLE,
                      50,15,650,25,hwnd,ID_TB5,NULL,NULL);
...
//         memset(music_lcdlist, 0, sizeof(music_lcdlist));
         ...
         wnd = CreateWindow(SCROLLBAR, L"SCROLLBAR_R", WS_TRANSPARENT|SBS_VERT|WS_OWNERDRAW|SBS_BOTTOM_ALIGN|SBS_NOARROWS|WS_CLIPSIBLINGS, 
...
//                  x_mbstowcs(wbuf, music_lcdlist[play_index], FILE_NAME_LEN);
                  x_mbstowcs(wbuf, &music_playlist[play_index][11], FILE_NAME_LEN-11);
...
//        color_bg = GetPixel(hdc, 300, 200);
      ...
      case WM_ERASEBKGND:
      {
         ...
         return TRUE;
       #else
         RECT rc_title = {0, 0, GUI_XSIZE, 55};
         RECT rc_title_grad = {0, 55, GUI_XSIZE, 5};
         RECT rc_lyric = {0, 55, GUI_XSIZE, 396};
